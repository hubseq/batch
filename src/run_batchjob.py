...
def run_batchjob( args_json ):
    ...
    io_json_name = module_name+'.'+unique_id+'.io.json'
    with open(io_json_name,'w') as fout:
        json.dump(io_json, fout)
    print('ARGS JSON: '+str(args_json))
    
    # upload IO JSON to module directory
    io_json_remote_folder = file_utils.uploadFile(io_json_name, os.path.join( module_utils.getModuleDirectory(), module_name, 'io/' ))
